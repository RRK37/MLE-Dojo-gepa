...
class Agent:
    ...
    def _draft(self) -> Node:
        # Update learned insights from previous solutions
        ...
        # Add learned guidance to instructions if available
    ...
    def _update_learned_insights(self):
        """Update learned insights by analyzing solution patterns every 3 steps."""
        current_step = len(self.journal.nodes)
        
        # Only run analysis every 3 steps and if we have at least 3 solutions
        if current_step - self.last_analysis_step < 3 or current_step < 3:
            return
        
        logger.info(f"Running pattern analysis at step {current_step}")
        self.last_analysis_step = current_step
        
        try:
            # Collect all solutions with their metrics
            solutions = []
            for node in self.journal.nodes:
                if node.code and not node.is_buggy:
                    analysis = self.solution_analyzer.analyze_solution(
                        code=node.code,
                        score=node.metric.value if hasattr(node.metric, 'value') else 0.0,
                        execution_time=0.0,  # Could track this if available
            ...
            # Cluster solutions to find patterns
            clusters = self.pattern_discovery.cluster_solutions(
                solutions,
                n_clusters=min(3, len(solutions))
            )
            
            # Analyze correlations between patterns and performance
            patterns = self.correlation_analyzer.analyze_clusters(clusters)
            
            # Generate insights for prompt injection
            self.learned_insights = self.insight_generator.generate_insights(patterns)
            self.learned_guidance = self.insight_generator.generate_guidance(patterns)
            
            # Analyze temporal patterns (stagnation, diversity)
            temporal_insights = self.temporal_analyzer.analyze_evolution(
                [s["score"] for s in solutions],
                [s["features"] for s in solutions]
            )
            
            if temporal_insights:
                self.learned_insights += "\n\n**Temporal Analysis:**\n" + temporal_insights
            
            logger.info(f"Pattern analysis complete. Generated {len(self.learned_guidance)} guidance points.")
            
        except Exception as e:
            logger.warning(f"Pattern analysis failed: {e}")
            # Don't crash the agent if analysis fails
            pass